...
from transformers.utils import TransformersKwargs, auto_docstring, check_torch_load_is_safe, logging as hf_logging
...
from models.FOA_IV_encoder import NeuralIV
from models.Classic_IV import ClassicIV

from models.Depth_positional_encoding import PositionEmbeddingSine3D
...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniAudioEncoderConfig):
        ...
        self.foa_output_dim = 1203 # 401 * 3
        ...
        self.ln_post = nn.LayerNorm(config.d_model + self.foa_output_dim)
        self.avg_pooler = nn.AvgPool1d(2, stride=2)
        self.proj = nn.Linear(config.d_model + self.foa_output_dim, config.output_dim)

        # --- FOA Encoder (Classic or Neural) ---
        # self.foa_iv_encoder = NeuralIV(
        #     input_channels=4,
        #     out_channels=401,
        #     params=[(10, 5), (3, 2), (3, 2), (3, 2), (3, 2), (2, 2), (2, 2)],
        #     use_norm=True,
        # )
        self.foa_iv_encoder = ClassicIV()
        # ---------------------------------------
    ...
    def forward(
        ...
        **kwargs,
        ...
        raw_wav (`torch.FloatTensor` of shape `(batch_size, 4, time)`, *optional*):
            The raw waveform input for FOA (First-Order Ambisonics) encoding.
        raw_wav_lens (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            The length of valid raw waveform input.
        """
        ...
        if raw_wav is not None:
            # IV encoder forward: [B, 4, T] -> [B, T_foa, 1203]
            foa_embeds = self.foa_iv_encoder(raw_wav)
        # ----------------------------

        foa_total_stride = 5*2*2*2*2*2*2

        token_audio_list = []
        for i, each_audio_states in enumerate(hidden_states_list):
            # ---- Fusion with FOA Embeddings ----
            if foa_embeds is not None:
                curr_foa = foa_embeds[i]

                if raw_wav_lens is not None:
                    valid_foa_len = raw_wav_lens[i] // foa_total_stride
                    valid_foa_len = max(1, min(curr_foa.shape[0], valid_foa_len)) # For safety, avoid 0 or out-of-bound
                    curr_foa = curr_foa[:valid_foa_len]

                target_len = each_audio_states.shape[0]
                curr_foa_len = curr_foa.shape[0]

                if target_len != curr_foa_len:
                    # logging.warning(f"FOA length {curr_foa_len} different from audio token length {target_len}, resampling FOA.")
                    curr_foa_t = curr_foa.transpose(0, 1).unsqueeze(0)
                    curr_foa_t = F.interpolate(
                        curr_foa_t,
                        size=target_len,
                        mode="linear",
                        align_corners=False,
                    ...
                    curr_foa = curr_foa_t
                each_audio_states = torch.cat([each_audio_states, curr_foa.to(each_audio_states.dtype)], dim=-1)
            else:
                pad_foa = torch.zeros(
                    (each_audio_states.shape[0], self.foa_output_dim),
                    dtype=each_audio_states.dtype,
                    device=each_audio_states.device,
                )
                each_audio_states = torch.cat([each_audio_states, pad_foa], dim=-1)
            # -------------------------------
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniVisionEncoderConfig, *inputs, **kwargs) -> None:
        ...
        self.pcd_pe = PositionEmbeddingSine3D(embedding_size=config.hidden_size, temperature=10000, n_points=1)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, point_clouds: torch.Tensor = None, **kwargs) -> torch.Tensor:
        """
        Args:
            ...
            grid_thw (`torch.Tensor` of shape `(num_images_or_videos, 3)`):
                The temporal, height and width of feature shape of each image in LLM.
            point_clouds (`torch.Tensor` of shape `(batch_size, num_points, 3)`, *optional*):
        Returns:
        ...
        if point_clouds is not None:
            if point_clouds.shape[0] != grid_thw.shape[0]:
                raise ValueError(f"Batch mismatch: point_clouds has {point_clouds.shape[0]} samples but grid_thw has {grid_thw.shape[0]}")
            
            pe_feat_list = []
            
            for i, (t, h, w) in enumerate(grid_thw):
                assert t == 1, "Only support bs=1 now"
                world_coords = point_clouds[i] # (N, 3)
                
                if h < w:
                    world_coords = world_coords.reshape(1, -1, 672, 3)
                else:
                    world_coords = world_coords.reshape(1, 672, -1, 3)
                assert torch.isfinite(world_coords).all(), "Tensor contains NaN or Inf values"
                # Pooling
                world_coords_map = world_coords.permute(0, 3, 1, 2).float()
                ...
                # Normalize & Encode
                mean_xyz = world_coords_avg.reshape(3, -1).mean(1).view(3, 1, 1)
                world_coords_avg = (world_coords_avg[0] - mean_xyz) / 0.001 # (3, h, w)
                
                world_coords_avg = world_coords_avg.round()[None].permute(0, 2, 3, 1).flatten(1, 2)
                pe_one = self.pcd_pe(world_coords_avg)[0] # (seq_len_single, dim)

                if t > 1:
                    raise NotImplementedError("Point cloud positional embedding for video input is not implemented yet.")

                pe_feat_list.append(pe_one)

            pe_feat = torch.cat(pe_feat_list, dim=0)
            
            pe_feat = pe_feat.reshape(seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1)
            pe_feat = pe_feat[window_index, :, :]
            pe_feat = pe_feat.reshape(seq_len, -1)

            hidden_states = hidden_states + pe_feat.to(hidden_states.dtype)
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, point_clouds: Optional[torch.FloatTensor] = None):
        """
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        raw_wav_lens (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            The length of valid raw waveform input.
        point_clouds (`torch.FloatTensor` of shape `(batch_size, num_points, 3)`, *optional*):
            The point clouds input for 3D understanding.
        rope_deltas (`torch.LongTensor` of shape `(batch_size, )`, *optional*):
            The rope index difference between sequence length and multimodal rope.
        ...
        if pixel_values is not None:
            image_embeds = self.get_image_features(pixel_values, image_grid_thw, point_clouds=point_clouds)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        model_inputs["raw_wav"] = raw_wav
        model_inputs["raw_wav_lens"] = raw_wav_lens
