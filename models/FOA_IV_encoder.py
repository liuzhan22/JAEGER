...
class NeuralIV(nn.Module):
    def __init__(self, input_channels, out_channels, params, use_norm=True):
        super().__init__()
        ...
        for i, (kernel_size, stride) in enumerate(params):
            ...
            if self.use_norm:
                # GroupNorm with 1 group is equivalent to LayerNorm for [B, C, L] inputs
                self.conv_layers.append(nn.GroupNorm(1, out_channels))
    ...
    def forward(self, raw_wav):
        ...
        for layer in self.conv_layers:
            x = layer(x) # [B*C, out_channels, T_final]

        x = x.transpose(1, 2) # [B*C, T_final, out_channels]
        x = x.view(B, C, -1, self.out_channels) # [B, C, T_final, out_channels]

        main_spec = x[:, 0:1, :, :]
        other_spec = x[:, 1:, :, :]

        I = main_spec * other_spec # [B, C-1, T_final, out_channels], C-1=3 for FOA
        for layer in self.linear_layers:
            I = layer(I) # [B, C-1, T_final, out_channels]

        I = I.transpose(1, 2)
        y = I.reshape(B, -1, self.out_channels * (self.input_channels - 1)) # [B, T_final, out_channels*(C-1)]
